...
const UploadBox = ({ 
  ...
  maxSize: maxSizeProp,
  maxFileSize,
  // Support both acceptedTypes and acceptedFileTypes props
  ...
  const validateFile = useCallback(async (file) => {
    ...
    try {
      ...
      if (minDimensions) {
        if (dimensions.width < (minDimensions.width || 0) || dimensions.height < (minDimensions.height || 0)) {
          errors.push(`Image must be at least ${minDimensions.width || 0}x${minDimensions.height || 0} pixels`);
        }
      }
      if (maxDimensions) {
        if (dimensions.width > (maxDimensions.width || Infinity) || dimensions.height > (maxDimensions.height || Infinity)) {
